...
module.exports.run = async (client, message, args) => {
 ...
 //   let ppv = args[1];
    ...
    let frNotes = notes.notes.filter((n) => n.codeMatiere == 'ALL2').map((v) => v.valeur)
    let frDevoir = notes.notes.filter((n) => n.codeMatiere == 'ALL2').map((v) => v.devoir)
    let frTypeDevoir = notes.notes.filter((n) => n.codeMatiere == 'ALL2').map((v) => v.typeDevoir)
    let frnoteSur = notes.notes.filter((n) => n.codeMatiere == 'ALL2').map((v) => v.noteSur)
    let frMoyenneClasse = notes.notes.filter((n) => n.codeMatiere == 'ALL2').map((v) => v.moyenneClasse)
    let frmin = notes.notes.filter((n) => n.codeMatiere == 'ALL2').map((v) => v.minClasse)
    let frmax = notes.notes.filter((n) => n.codeMatiere == 'ALL2').map((v) => v.maxClasse)
    let frlibelleMatiere = notes.notes.filter((n) => n.codeMatiere == 'ALL2').map((v) => v.libelleMatiere)
    let frDate = notes.notes.filter((n) => n.codeMatiere == 'ALL2').map((v) => v.date)

    let nb = frDevoir.length - 1;

    if (args[0] > nb || !frnoteSur) {
        return message.channel.send(`Ce message n'existe **pas**\nMessage(s) disponible(s)**:**\n**0** - **${nb}**`)
      }

    const embedFrancais = new MessageEmbed()
    .setColor('#25ff')
    .setTitle(`__Affiche les notes d'allemand de ${user.tag}__`)
...
module.exports.help = {
    ...
    description: "Permet de recupéres ses notes d\'Allemand.",
    usage: "<nombre>"
};