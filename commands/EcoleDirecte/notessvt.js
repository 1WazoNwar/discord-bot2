...
module.exports.run = async (client, message, args) => {
 ...
 //   let ppv = args[1];
    ...
    let frNotes = notes.notes.filter((n) => n.codeMatiere == 'SVT').map((v) => v.valeur)
    let frDevoir = notes.notes.filter((n) => n.codeMatiere == 'SVT').map((v) => v.devoir)
    let frTypeDevoir = notes.notes.filter((n) => n.codeMatiere == 'SVT').map((v) => v.typeDevoir)
    let frnoteSur = notes.notes.filter((n) => n.codeMatiere == 'SVT').map((v) => v.noteSur)
    let frMoyenneClasse = notes.notes.filter((n) => n.codeMatiere == 'SVT').map((v) => v.moyenneClasse)
    let frmin = notes.notes.filter((n) => n.codeMatiere == 'SVT').map((v) => v.minClasse)
    let frmax = notes.notes.filter((n) => n.codeMatiere == 'SVT').map((v) => v.maxClasse)
    let frlibelleMatiere = notes.notes.filter((n) => n.codeMatiere == 'SVT').map((v) => v.libelleMatiere)
    let frDate = notes.notes.filter((n) => n.codeMatiere == 'SVT').map((v) => v.date)
    ...
    .setColor('#25ff')
    .setTitle(`__Affiche les notes de SVT de ${user.tag}__`)
...
module.exports.help = {
    ...
    description: "Permet de recupéres ses notes de SVT.",
    usage: "<nombre>"
};